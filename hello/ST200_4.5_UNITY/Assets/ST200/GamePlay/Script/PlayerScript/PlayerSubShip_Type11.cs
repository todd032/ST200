...
public class PlayerSubShip_Type11 : PlayerSubShip {
	
	float aiprocesstimer = 0f;
	protected float m_ItemSpawnTimer = 0f;
	...
	public override void Process(float _timer)
	{
		...
		if(m_CurHealth > 0f)
		{
			if(m_ItemSpawnTimer > m_StatInfo.SpecialGaugeMax)
			{
				SpawnItem();
				m_ItemSpawnTimer -= m_StatInfo.SpecialGaugeMax;
			}else
			{
				m_ItemSpawnTimer += _timer * GaugeSpeed / 3f;
			}
		}
	}
	...
	{
		StageItemManager.Instance.TrySpawnItem(TeamIndex, (int)m_StatInfo.SpecialEffectValue4, m_ShootTransform[1].position);
	}
}