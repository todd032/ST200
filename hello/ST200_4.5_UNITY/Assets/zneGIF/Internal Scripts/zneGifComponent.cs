...
public class zneGifComponent : MonoBehaviour
{
	...
	//public UITexture[] _targets;

	public bool NGUIMode = true ;
    ...
    internal virtual void GifComponent_Start()
    {
        ...
        if ( (_targets == null || _targets.Length == 0) && !Application.isPlaying)
        {
			...
			if(NGUIMode){
				UITexture targetUITexture = GetComponent<UITexture>() as UITexture ;
				if (gameObject.renderer != null || targetUITexture != null)
					_targets = new GameObject[] { gameObject };
				else
					SetError("No have target!");
			}else{
				if (gameObject.renderer != null || gameObject.guiTexture != null)
					_targets = new GameObject[] { gameObject };
				else
					SetError("No have target!");
			}
    ...
    internal virtual void SetTextureTo(GameObject target, Texture2D texture)
    {
        if (target != null)
        {
			/*
            //  Set to GUI Texture
            if (target.guiTexture != null)
            {
                target.guiTexture.texture = texture;
            }
			*/

			UITexture targetUITexture = target.GetComponent<UITexture>() as UITexture ;
			
			//  Set to GUI Texture
			if (targetUITexture != null)
			{
				targetUITexture.mainTexture = texture ;
			}
