...
public class SubmarinePowerShotEffectCellSpriteObject : MonoBehaviour {
	...
	protected virtual IEnumerator Move() {
		...
		while(_fsmState == FSMState.Move) {
			...
			if(!isDone){
				...
				//_thisTransform.position = endPosition;//Vector3.Lerp(startPosition,endPosition,t) ;
				
				if(t > 1f)// _thisTransform.position == endPosition) {
				{
