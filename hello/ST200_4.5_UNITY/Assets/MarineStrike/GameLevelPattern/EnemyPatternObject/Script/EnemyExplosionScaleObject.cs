...
public class EnemyExplosionScaleObject : EnemyObject
{
	
	public GameObject _damageMarkIcon ;
	
	public float _explosionDistance = 0.26f;
	...
	protected  virtual IEnumerator Idle(){
		...
		float scaleval = Random.Range(0.5f, 0.7f);
		transform.localScale = Vector3.one * 0.7f;
		while(_enemyState == EnemyState.Idle) {
			
			Vector2 pos = Camera.main.WorldToScreenPoint(_thisTransform.position);
			transform.localScale += Vector3.one * scaleval * Time.deltaTime;
			if(pos.x < Screen.width * _explosionDistance ||
			   transform.localScale.x > 1.5f) _enemyState = EnemyState.Explosion;
...
protected virtual IEnumerator Crash(){
	...
	while(_enemyState == EnemyState.Crash) {
		...
		Vector2 pos = Camera.main.WorldToScreenPoint(_thisTransform.position);
		if(pos.x < Screen.width * _explosionDistance ) _enemyState = EnemyState.Explosion;
...
*/
	...
	protected virtual IEnumerator Explosion(){
		...
		//Attack Explosion Effect
		GameObject _go = PoolManager.Spawn("EnemyExplosionEffectObject") ;
		EnemyExplosionEffectObject _enemyExplosionEffectObject = _go.GetComponent<EnemyExplosionEffectObject>() as EnemyExplosionEffectObject ;
		_enemyExplosionEffectObject.ResetEffectObject(_thisTransform.position) ;
		_enemyExplosionEffectObject.StartAction() ;
		//
		...
		/*
	while(_enemyState == EnemyState.Explosion) {
